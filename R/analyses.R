...
# If the surface DIN > 1 uM, assign nutricline depth of 0 m
clean_data_for_gam <- function(data) {
  data_for_gam <- data %>% dplyr::mutate(NO3 = ifelse(DIN < 0.1, 0.1, DIN),
                                         ...
                                         logPO4 = log(PO4),
                                         SST = Temperature,
                                         Nutcline_1uM_interp = ifelse(DIN > 1.0, 0, Nutricline_1uM_Interp)) %>% dplyr::select(logCP, logNP, logCN,
                                                                              SST, logNO3, logPO4,
...
# Function to make CNP gam p-value summary table for high latitude for ver1 with 2 variables
make_CNP_pval_highlat_ver1 <- function(data = POM_highlat_ver1_surface_binned_gam) {
  xvar1 <- "SST"
  xvar2 <- "logNO3"  
  cp_pvalue_highlat <- get_pval_gam(b1_2vars(xvar1, xvar2, "logCP", data))
  cp_pvalue_highlat<- R.utils::insert(cp_pvalue_highlat,ats=c(3),values=c("n.s.","n.s."))  
  np_pvalue_highlat <- get_pval_gam(b1_2vars(xvar1, xvar2, "logNP", data))
  np_pvalue_highlat<- R.utils::insert(np_pvalue_highlat,ats=c(3),values=c("n.s.","n.s."))  
  cn_pvalue_highlat <- get_pval_gam(b1_2vars(xvar1, xvar2, "logCN", data))
  cn_pvalue_highlat<- R.utils::insert(cn_pvalue_highlat,ats=c(3),values=c("n.s.","n.s.")) 
  CNP_highlat_pval <- as.data.frame(cbind(cp_pvalue_highlat,np_pvalue_highlat,cn_pvalue_highlat))
  rownames(CNP_highlat_pval) <- c("SST", "Nitrate","Nutricline", "Nutcline x Nutlim","Total")
  colnames(CNP_highlat_pval) <- c('C:P','N:P','C:N')  
  CNP_highlat_pval
...
# Function to calculate deviance explained in high latitudes with 32 variables (SST, NO3)
calc_devexpl_highlat_ver1 <- function(xvar1, xvar2, yvar, data) {
  result_highlat <- deviance2variables(xvar1, xvar2, yvar, data)
  result_highlat<- R.utils::insert(result_highlat,ats=c(3),values=c(0,0))  
  result_highlat
}
...
# Function to make CNP deviance explained summary table for high latitude for 
# GO-POPCORN ver1
# SST and Nitrate only (Nutricline is 0 everywhere so remove from predictor)

make_CNP_devexpl_highlat_ver1 <- function(data = POM_highlat_ver1_surface_binned_gam) {
  xvar1 <- "SST"
  xvar2 <- "logNO3" 
  result_cp_highlat <- calc_devexpl_highlat_ver1(xvar1, xvar2, "logCP", data)
  result_np_highlat <- calc_devexpl_highlat_ver1(xvar1, xvar2, "logNP", data)
  result_cn_highlat <- calc_devexpl_highlat_ver1(xvar1, xvar2, "logCN", data)
  CNP_highlat_devexpl <- as.data.frame(cbind(result_cp_highlat[1:5],result_np_highlat[1:5],result_cn_highlat[1:5]))
  rownames(CNP_highlat_devexpl) <- c("SST", "Nitrate","Nutricline", "Nutcline x Nutlim",'Total')
  colnames(CNP_highlat_devexpl) <- c('C:P','N:P','C:N')  
  round(CNP_highlat_devexpl, digits = 3)
